...
JSON.parse = function () {
  ...
  if (typeof arguments[0] === 'string' && arguments[0].includes('FE_what_to_watch')) {
    try {
      console.log('[PulseTube TV] FE_what_to_watch JSON intercepted. Size: ' + arguments[0].length);
      console.log('[PulseTube TV] DUMP_SNIPPET: ' + arguments[0].substring(0, 2000));
    } catch(e) {}
  }
...
function hqify(items) {
  return; // Disabled: HQ thumbnails consume too much RAM on Android TV, causing extreme lag
}

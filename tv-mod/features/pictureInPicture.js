...
function pipLoad() {
    if (!window._yttv) {
        console.log('[PulseTube TV] window._yttv is not defined yet, scheduling pipLoad retry...');
        return setTimeout(pipLoad, 250);
    }
    const mappings = Object.values(window._yttv).find(a => a && a.mappings);
    if (!mappings) {
        console.log('[PulseTube TV] window._yttv mappings not found yet, scheduling pipLoad retry...');
        return setTimeout(pipLoad, 250);
    }

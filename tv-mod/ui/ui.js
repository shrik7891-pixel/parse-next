...
import { pipToFullscreen, enablePip } from '../features/pictureInPicture.js';
...
function execute_once_dom_loaded() {
  ...
  if (existingStyle) {
    ...
    if (nonceEl && nonceEl.getAttribute('nonce')) {
...
`;
  ...
  var eventHandler = (evt) => {
    ...
    if (evt.keyCode == 403) {
      ...
      // Yellow button - Toggle Playback Speed (1x -> 1.5x -> 2x)
      if (evt.type === 'keydown') {
        evt.preventDefault();
        evt.stopPropagation();
        ...
        if (video) {
          ...
          else if (video.playbackRate === 1.5) video.playbackRate = 2.0;
          else video.playbackRate = 1.0;
          showToast('TizenTube', `Playback Speed: ${video.playbackRate}x`);
        }
      }
      return false;
    } else if (evt.keyCode == 406 || evt.keyCode == 191) {
      // Blue button - Picture in Picture
      if (evt.type === 'keydown') {
        evt.preventDefault();
        evt.stopPropagation();
        if (window.h5vcc && window.h5vcc.tizentube && window.h5vcc.tizentube.EnterPIP) {
           window.h5vcc.tizentube.EnterPIP();
        } else {
           enablePip();
        }
        showToast('TizenTube', 'Picture-in-Picture Activated');
      }
      return false;
  ...
  // Force User Requested Settings
  try {
      configWrite('enableHideWatchedVideos', true);
      
      // Force location to US (gl=US) and default language to English (hl=en)
      let currentPref = document.cookie.split('; ').find(row => row.startsWith('PREF=')) || 'PREF=';
      if (!currentPref.includes('gl=US')) {
          let newPref = currentPref;
          if (newPref.includes('gl=')) {
              newPref = newPref.replace(/gl=[A-Z]{2}/, 'gl=US');
          } else {
              newPref += (newPref.endsWith('=') ? 'gl=US' : '&gl=US');
          }
          document.cookie = `${newPref}; expires=${new Date(Date.now() + 315360000000).toUTCString()}; domain=.youtube.com; path=/`;
      }
  } catch (e) {
      console.warn('[PulseTube TV] Failed to apply forced settings', e);
  }

  // Auto-Boot directly into Pulse All
  setTimeout(() => {
    try {
      console.log('[PulseTube TV] Auto-booting into Pulse All...');
      resolveCommand({
        browseEndpoint: {
          browseId: 'FE_pulse_feeds'
        }
      });
      showToast('PulseTube', 'Booted into Pulse All');
    } catch (e) {
      console.warn('[PulseTube TV] Auto-boot failed', e);
    }
  }, 1000);
